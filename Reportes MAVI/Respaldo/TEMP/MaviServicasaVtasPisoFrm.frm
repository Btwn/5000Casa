...
Nombre=Info.UENMAVI+Si(Mavi.ServicasaExplora=1,<T> EXPLORA <T>,<T> SERVICASA <T>)+<T> SUCURSAL <T>+Sucursal+<T>                                                                                                  <T>+Ahora
Icono=746
Menus=S
BarraHerramientas=S
Modulos=VTAS
...
ListaCarpetas=Servicasa<BR>Detalle<BR>Sucursal
...
ListaAcciones=AbrirMovSC<BR>Nuevo<BR>InfoCteSC<BR>MovPosSC<BR>MovBitacoraSC<BR>MovTiempoSC<BR>AgregarEventoSC<BR>ExploraSCT<BR>ServicredCred<BR>HistoricoUnicajaSC<BR>ActualizarSC<BR>PersonalizarSC<BR>AcercadeSCV<BR>Cerrar<BR>RepAnuales<BR>HistoricoSolicitudes<BR>Consulta
PosicionSec1=41
PosicionSec2=497
AccionesDivision=S
Comentarios=Usuario
...
PosicionCol1=642
ExpresionesAlMostrar=Asigna(Info.UEN,SQL(<T>Select UEN from usuario where usuario=:tval1<T>,usuario))<BR>Asigna(Info.UENMAVI,SQL(<T>Select Nombre from UEN Where Uen=:nval1<T>,Info.UEN))<BR>Asigna(Mavi.Sucursal,sucursal)
...
MenuLocal=S
ListaAcciones=AbrirMovDSC<BR>NuevoDSCT<BR>InfoCteDSC<BR>MovPosDSC<BR>MovBitacoraDCS<BR>MovTiempoDSC<BR>AgregarEventoDSC<BR>ActualizarDSC<BR>PersonalizarDSC
ConFuenteEspecial=S
...
FiltroFechas=S
FiltroSituacion=S
FiltroListaEstatus=(Todos)<BR>PENDIENTE<BR>CANCELADO<BR>CONCLUIDO<BR>SINAFECTAR<BR>AUTORIZADO<BR>RECHAZADO
FiltroEstatusDefault=PENDIENTE
FiltroFechasCampo=FF
...
ListaEnCaptura=SucursalOrigen<BR>Cliente<BR>Nombre<BR>CANALVENTA<BR>Condicion<BR>SolRefinanciamiento<BR>ImporteTotal<BR>TiempoTotalTranscurrido<BR>Estatus<BR>Situacion<BR>Reactivacion<BR>Grupo<BR>Relacionado<BR>Cita Supervisión<BR>%Supervision<BR>ReferenciaAnterior<BR>FechaAlta<BR>FF<BR>FUM<BR>Agente<BR>CategoriaVenta<BR>Calificacion<BR>Observaciones<BR>SucursalVenta<BR>Almacen<BR>Fech. Alta. a Fech. Ult. Mod.<BR>Fech. Ult. Mod. a Fech. Act.
...
Activo=S
Visible=S
Expresion=Ejecutar(<T>\\172.16.202.2\Versiones\3100 Capacitacion\PlugIns\Inte601.exe <T>+MaviServiCasaVtasPisoVis:Cliente+<T> N<T>)
[Servicasa.FechaAlta]
Carpeta=Servicasa
Clave=FechaAlta
Editar=S
...
Activo=S
Visible=S
Expresion=Ejecutar(<T>C:\Archivos de programa\RF\Inte602.exe <T>+MaviServiCasaVtasPisoVis:Cliente+<T> N<T>)
