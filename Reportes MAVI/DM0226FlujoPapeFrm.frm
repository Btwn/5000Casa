...
Activo=S
Visible=S
Expresion=Si<BR>  DM0226InvVis:Inv.Mov =<T>Solicitud Papeleria<T><BR>Entonces<BR>    Asigna(Afectar.Modulo, <T>INV<T>)<BR>    Asigna(Afectar.ID, DM0226InvVis:Inv.ID)<BR>    Asigna(Afectar.Mov, DM0226InvVis:Inv.Mov)<BR>    Asigna(Afectar.MovID, DM0226InvVis:Inv.MovID)<BR>    Asigna(Afectar.Base, <T>Todo<T>)<BR>    Asigna(Afectar.GenerarMov, <T><T>)<BR>    Asigna(Afectar.FormaCaptura, <T>DM0226PruebaInv<T>)<BR>    Asigna(Info.Respuesta2,DM0226InvVis:Inv.Observaciones)<BR><BR>    Afectar(Afectar.Modulo, Afectar.ID, Afectar.Mov, Afectar.MovID, Afectar.Base, Afectar.GenerarMov, Afectar.FormaCaptura,Info.Respuesta2)<BR>     Asigna(Info.Respuesta1,SI(SQL(<T>Select ID From Inv Where Id=:nId and Estatus=:tEst<T>,DM0226InvVis:Inv.ID,<T>PENDIENTE<T>),<T>YES<T>,<T>NO<T>))<BR>                                    <CONTINUA>
